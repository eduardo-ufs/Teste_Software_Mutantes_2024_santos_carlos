...
# defines first unit test for testing application version
def test_version():
...
# pytest fixtures are functions that are used to manage app states and dependencies. They can provide data for testing and a wide range of value types when explicitly called by our testing software. You can use the mock data that fixtures create across multiple tests.
@pytest.fixture
# this fixture creates and returns temporary JSON File - "db_file" with a single to-do list item. The tmp_path is a pathlib.Path object that pytest uses to provide a temporary directory for testing purposes
def mock_json_file(tmp_path):
    todo = [{"Description": "Get milk", "Priority": 2, "Done": False},
            {"Description": "XXXGet milkXXX", "Priority": 2, "Done": False}]
...
@pytest.mark.parametrize(                                                       # The @pytest.mark.parametrize() decorator marks test_add() for parametrization. When pytest runs this test, it calls test_add() two times. Each call uses one of the parameter sets test_data1, test_data2
    # holds descriptive names for the two required parameters and also a descriptive return value name. The test_add() has the same parameters
...
# this functions first parameter has the same name as the fixture defined
def test_add(mock_json_file, description, priority, expected):
    ...
    # reads the to-do list from the temporary database and stores it in read variable
    read = todoer._db_handler.read_todos()
    # asserts that the length of the to-do list is 2 because mock_json_file() returns a list with one item, and now the test_add() adds a second item to the list.
